import { School, Users, CreditCard, TrendingUp, AlertTriangle, CheckCircle, Clock, ArrowUpRight } from "lucide-react"
...
export default async function AdminDashboardPage() {
  ...
  const expiringSoon = await getExpiringSubscriptions()
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">{stats.monthlyRevenue.toLocaleString("tr-TR")} TL</p>
                <div className="mt-1 flex items-center gap-1 text-xs text-emerald-400">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="border-slate-800 bg-slate-900">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-lg font-semibold text-white">Son Kayıt Olan Okullar</CardTitle>
          ...
          <CardContent className="space-y-3">
            {recentTenants.length === 0 ? (
                <div className="text-center text-sm text-slate-500 py-4">Henüz okul kaydı yok</div>
            ) : (
                recentTenants.map((tenant) => (
                <div key={tenant.id} className="flex items-center justify-between rounded-lg bg-slate-800/50 p-3">
                    <div className="flex items-center gap-3">
                    <Avatar className="h-10 w-10">
                        <AvatarFallback className="text-white text-xs" style={{ backgroundColor: tenant.primaryColor }}>
        ...
        <Card className="border-slate-800 bg-slate-900">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-lg font-semibold text-white">Son Ödemeler</CardTitle>
            <Button variant="ghost" size="sm" asChild className="text-blue-400 hover:text-blue-300">
          ...
          <CardContent className="space-y-3">
            {recentPayments.length === 0 ? (
                <div className="text-center text-sm text-slate-500 py-4">Henüz ödeme yok</div>
            ) : (
                recentPayments.map((payment) => (
                <div key={payment.id} className="flex items-center justify-between rounded-lg bg-slate-800/50 p-3">
                    <div className="flex items-center gap-3">
                    ...
                    <p className="font-semibold text-emerald-400">+{payment.amount.toLocaleString("tr-TR")} TL</p>
                    <p className="text-xs text-slate-400">{new Date(payment.paidAt).toLocaleDateString("tr-TR")}</p>
                    </div>
                </div>
                ))
            )}
        ...
        <Card className="border-slate-800 bg-slate-900 lg:col-span-2">
          ...
          <CardContent>
            {expiringSoon.length === 0 ? (
              <div className="py-8 text-center text-slate-400">Yaklaşan abonelik süresi bulunmuyor</div>
            ) : (
              <div className="space-y-3">
                {expiringSoon.map((sub) => {
                  ...
                  return (
                    <div key={sub.id} className="flex items-center justify-between rounded-lg bg-amber-500/10 p-3">
                      <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10">
                          <AvatarFallback
                            className="text-white text-xs"
                            style={{ backgroundColor: sub.tenant?.primaryColor }}
                          >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400">
                            {sub.plan?.name} - {sub.billingPeriod === "monthly" ? "Aylık" : "Yıllık"}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <Badge className="bg-amber-500/20 text-amber-400">{daysLeft} gün kaldı</Badge>
